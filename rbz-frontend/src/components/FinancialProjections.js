...
const FinancialProjections = ({ onComplete }) => {
    ...
    const calculateRatios = (year, field, value) => {
        ...
        setProjections(prev => {
            ...
            // Formula: Total Assets = Total Equity + Current Liabilities
            // Apply to the specific year being edited

            // Re-fetch equity/liabilities for the current year context
            let equity = parseFloat(currentYearProj.totalEquity || 0);
            let liabilities = parseFloat(currentYearProj.currentLiabilities || 0);
